...
::: {.card title="Annual max Siegel et al. 2023 pred. daily stream temp"}
...
::: {.card title="Annual min USGS obs mean daily Q"}
...
::: {.card title="Annual days above degC"}
...
```{r}
#| title: Annual max Siegel et al. 2023 pred. daily stream temp
plot_tmax(sites$site_no[1])
```

```{r}
#| title: Annual min USGS obs mean daily Q
...
```{r}
#| title: Annual days above degC
plot_tover(sites$site_no[1])
```

```{r}
#| title: Annual low flow quantiles - Q10, Q05, Q01
plot_qquant(sites$site_no[1])
```
...
```{r}
#| title: Annual days above degC
plot_tover(sites$site_no[2])
```

```{r}
#| title: Annual low flow quantiles - Q10, Q05, Q01
plot_qquant(sites$site_no[2])
```
