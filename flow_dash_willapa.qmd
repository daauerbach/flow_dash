...
makeplotly <- function(type, site, log10 = F){

  d <- usgs_q_dv |> filter(site_no==site) |> select(year, yday, {{ type }})
  vals <- d |> pull({{type}})
...
usgs_q_dv |> 
  filter(site_no == sites$site_no[1], 
         year != "2024"
         ) |>
...
```{r}
#| title: Mean daily Q
makeplotly(type = "q_dv_mean", site = sites$site_no[1], log10 = T)
```
...
#| title: Annual min Obs mean daily Q
usgs_q_dv |> 
  filter(site_no == sites$site_no[2], 
         year != "2024"
         ) |>
  slice_min(order_by = q_dv_mean, n = 1, by = year, with_ties = F) |>
  mutate(month = if_else(is.na(month), "med", month.abb[month]) |> 
...
```{r}
#| title: Mean daily Q
makeplotly(type = "q_dv_mean", site = sites$site_no[2], log10 = T)
```
