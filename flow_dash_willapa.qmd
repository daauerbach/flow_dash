...
pal <- set_names(
  c(
    # "#241E33", "#24213E", "#222548", "#1F2A52", "#18305A",
...
crc |>
  ...
  facet_wrap(~areaname + species, ncol = 3, dir = "v", scales = "free")

#Naselle 375, Willapa 424
crc |>
  filter(
    areacode == "375",
    between(statmonth, 6, 10)
  ...
  arrange(year, statmonth) |> 
  mutate(est_tot = cumsum(est), .by = c(areaname, species, year)) |> 

# ggplot(aes(statmonth, est, group = year)) +
#   geom_line() + geom_point() +
#   facet_wrap(~species, scales = "free")
  
# filter(species == "Chinook") |> 
# plotly::plot_ly(x = ~statmonth, y = ~est_tot, name = ~year, type = 'scatter', mode = 'lines+markers')
  
  #could also just take Oct
  slice_max(order_by = est_tot, n = 1, by = c(areaname, species, year)) |> 
  ggplot(aes(year, est_tot)) + geom_col() + facet_wrap(~species, scales = "free")

#summer median of dailymean
#could look at other flow stats
usgs_q_dv |> 
  filter(
    site_no == "12010000",
    between(month, 8, 10)
  ...
  summarise(
    across(q_dv_mean, quantile, probs = 0.5, na.rm = T), 
    .by = c(site_no, year)
  ) |> 
  left_join(
    crc |>
      ...
      summarise(est = sum(est), .by = c(species,year)) |> 
      mutate(year = as.character(year)),
    by = "year"
  ) |> 
  #filter(is.na(species))
  drop_na(species) |> #flow years outside CRC
  ggplot(aes(q_dv_mean, est)) + 
  geom_text(aes(label = year)) +
  facet_wrap(~species, scales = "free")

#maybe interesting? more catch ~ warmer streamtemp estimates
#but purely spurious and temporal?
nwfsc_st |> 
  filter(
    site_no == "12010000",
    between(month, 8, 10)
  ...
  summarise(
    across(st_pred, quantile, probs = 0.5, na.rm = T), 
    .by = c(site_no, year)
  ) |> 
  left_join(
    crc |>
  ...
  #filter(is.na(species))
  drop_na(species) |> #flow years outside CRC
  ggplot(aes(st_pred, est)) + 
  geom_text(aes(label = year)) +
  geom_smooth(method = 'lm') +
  facet_wrap(~species, scales = "free")

#anything lm(est ~ st_pred) for Chin seems captured by pure time
#perhaps not so for coho
#regress anomaly from long-term mean? [No, just rescales] or resid of lm(est ~ year)
crc |>
  filter(
    areacode == "375",
    between(statmonth, 6, 10)
...
#better!? lm_resid of lm(est ~ year) against median summer estimated stream temp
#controls Chin somewhat? 
nwfsc_st |>
  filter(
    site_no == "12010000",
    between(month, 8, 10)
  ...
  left_join(
    crc |>
      ...
      summarise(est = sum(est), .by = c(species,year)) |>
      #mutate(est_dev = (est - mean(est))/mean(est), .by = species) |> #ggplot(aes(year, est_dev, fill = species)) + geom_col() + facet_wrap(~species, scales = "free")
      nest(data = c(year, est), .by = species) |> 
      mutate(lm_resid = map(data, ~lm(est ~ year, data = .x)$resid)) |> 
      unnest(c(data, lm_resid)) |> 
      mutate(year = as.character(year))
    ,
    by = "year"
  ) |>
  #filter(is.na(species))
  drop_na(species) |> #flow years outside CRC
  #ggplot(aes(st_pred, est_dev)) +
  ggplot(aes(st_pred, lm_resid)) +
  geom_text(aes(label = year)) +
  geom_smooth(method = 'lm') +
  facet_wrap(~species, scales = "free")
